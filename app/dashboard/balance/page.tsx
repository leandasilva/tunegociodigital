...
const Balance: React.FC = () => {
  ...
  if (loadingMejores || loadingSales || loadingExpenses || loadingOrders) return 'Cargando...';

  const obtenerClientesUsuario = mejoresData?.obtenerClientesUsuario || [];

  const clienteGrafica = obtenerClientesUsuario.map((cliente: { razonsocial: any; totalGral: any; }) => ({
  ...
  const balance = {
    ...
    return (
      <>
        <div className="grid mt-7 grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        <CardDataStats
          title="Ganancia en el mes"
          total={`$${balance.ganancia}`}
          rate="0.43%"
          levelUp
        >
            <svg
