...
const EditarCajero = ({params}) => {
    ...
    const { data, loading, error } = useQuery(OBTENER_CAJERO, {
        ...
        const [ actualizarCajero ] = useMutation( ACTUALIZAR_CAJERO);
    ...
    const actualizarInfoCajero = async valores => {
        ...
        try {
            const { data} = await actualizarCajero({
                variables: {
                    id,
                    input: {
                        nombre, 
