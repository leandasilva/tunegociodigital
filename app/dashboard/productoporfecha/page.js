...
const ProductosPorFecha = () => {
            ...
            if (index % 3 === 0) {
              return (
                <View key={producto.id} style={{ width: '50%', marginBottom: 5 }}>
                  ...
                  <Text className={`${lusitana.className}`} style={{ fontSize: 25 }}>${producto.precio}</Text>
  ...
  return (
    <div>
      <>
        <h1 className={`${lusitana.className} text-2xl text-center text-gray-800 font-light mt-5 mb-5`}>¡¡Solo los productos modificados en dicha fecha podran ser impresos!!</h1>


        {obtenerProductoPorFecha.length === 0 ? (
          <p className={`${lusitana.className}mt-5 text-center text-2xl`}>No hay productos para imprimir</p>
        ) : (
          <div>
          <div> 
            <Button>
