...
const AUTENTICAR_ADMINISTRADOR = gql`
mutation AutenticarAdministrador($input: AutenticarInput) {
  autenticarAdministrador(input: $input) {
    token
    email
  }
}
...
const AUTENTICAR_CAJERO = gql`
mutation AutenticarCajero($input: AutenticarInput) {
  autenticarCajero(input: $input) {
    token
    email
  }
}
...
const Login = () => {
  ...
  const [mostrarPassword, setMostrarPassword] = useState(false); // Estado para alternar entre mostrar y ocultar la contraseña
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      email: Yup.string()
        ...
        .required('Email no puede ir vacío'),
      password: Yup.string()
  ...
  const [autenticarAdministrador]= useMutation(AUTENTICAR_ADMINISTRADOR);
  const [autenticarCajero]= useMutation(AUTENTICAR_CAJERO);
  ...
  const mostrarMensaje = () => {
    return (
      <div className="bg-white rounded-lg  py-2 px-3 w-full my-3 max-w-sm text-center mx-auto">
        <p>{mensaje}</p>
      </div>
    );
  };
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="flex flex-wrap items-center">
          <div className="hidden w-full xl:block xl:w-1/2">
            ...
            <Link href="/" passHref>
              <div className="flex items-center px-10 ml-20 mb-8 mt-8">
            ...
            <div className={`${montserrat.className} font-serif text-md mb-5`}>
              Sabías que si eres usuario puedes iniciar sesión desde tu celular ingresando a TuNegDigital.com!!
            </div>
